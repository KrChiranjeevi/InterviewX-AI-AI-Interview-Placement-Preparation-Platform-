...
// ─────────────────────────────────────────────────────────────────────
const W = {
  get bg() { return document.documentElement.classList.contains('light') ? '#F3F4F6' : '#020F08'; },
  get card() { return document.documentElement.classList.contains('light') ? '#FFFFFF' : '#051F11'; },
  get card2() { return document.documentElement.classList.contains('light') ? '#F9FAFB' : '#082E19'; },
  get border() { return document.documentElement.classList.contains('light') ? '#E5E7EB' : '#10562D'; },
  get accent() { return '#10B981'; },
  get accentLight() { return document.documentElement.classList.contains('light') ? '#059669' : '#34D399'; },
  get accentDark() { return '#047857'; },
  get accentGlow() { return document.documentElement.classList.contains('light') ? 'rgba(16, 185, 129, 0.15)' : 'rgba(16, 185, 129, 0.35)'; },
...
const TRACKS = [
  ...
  {
    id: 'turbo', name: 'Associate Analyst', pkg: '8.0 – 10.0 LPA', diff: 'Very Hard',
...
const PIPELINE = [
  ...
  { id: 'managerial', label: 'Managerial Interview' },
...
const TECH_FLOW = [
  { step: 'Welcome & Project Intro', q: "Welcome to your Deloitte Technical Interview! I'm Alex, your AI Technical Evaluator. Let's begin — please introduce yourself and walk me through your most significant project, including the problem it solves." },
...
const MANAGERIAL_FLOW = [
  { step: 'Leadership', q: "Tell me about a time you led a team through a difficult technical challenge." },
  { step: 'Conflict Resolution', q: "How do you handle disagreements with stakeholders or clients regarding technical decisions?" },
  { step: 'Decision Making', q: "Describe a situation where you had to make a quick decision with incomplete information." },
  { step: 'Business Scenarios', q: "If your project is running behind schedule and the client demands additional features, how do you manage the situation?" },
  { step: 'Professional Ethics', q: "What would you do if you discovered a critical security flaw in your team's code just hours before deployment?" }
];

const HR_FLOW = [
  { step: 'Self Introduction', q: "Hello and welcome! I'm Priya from Deloitte HR. I'm glad you're here today. To start, please tell me about yourself — your background, interests, and what brings you to Deloitte." },
  { step: 'Why Deloitte', q: "Wonderful! You've clearly done your research. What specifically draws you to Deloitte — our culture, technology stack, or growth opportunities? Why not other companies?" },
  ...
  { step: 'Career Goals', q: "Where do you see your career in 3–5 years? How does this role at Deloitte align with your long-term aspirations?" },
  { step: 'Flexibility & Availability', q: "Deloitte works on a global client model which may require relocating, working night shifts, or adapting to client time zones. Are you comfortable with these requirements?" },
  { step: 'Closing', q: "We're nearly done! Is there anything specific you'd like us to know about you that we haven't covered yet? And do you have any questions for me about Deloitte?" },
];
...
const BADGES = [
  ...
  { id: 'ready', name: 'Deloitte Ready', icon: FaShieldAlt, color: '#34D399', desc: 'Completed full simulation pipeline', earned: false },
  { id: 'fast', name: 'Fast Learner', icon: FaBolt, color: '#A78BFA', desc: 'Improved 15%+ across attempts', earned: true },
];

const HISTORY = [
  { date: '2025-11-10', track: 'Technology Consulting', oa: '71%', coding: '75%', tech: '68%', hr: '80%', overall: '73%', status: 'Not Selected', hire: 35 },
  { date: '2026-02-18', track: 'Analyst', oa: '84%', coding: '88%', tech: '81%', hr: '87%', overall: '85%', status: 'Borderline', hire: 60 },
  { date: '2026-07-05', track: 'Associate Analyst', oa: '95%', coding: '97%', tech: '93%', hr: '96%', overall: '95%', status: 'Selected', hire: 95 },
];

const COMMITTEE = [
  { role: 'Assessment Panel', icon: FaChartLine, color: '#818CF8', verdict: 'Strong Hire', rating: 95, comment: 'Exceptional cognitive performance across all four sections. 95% accuracy with optimal time management — significantly above Associate Analyst benchmark. Code solution passed all 4/4 test cases with O(N) complexity.' },
  { role: 'Technical Panel', icon: FaLaptopCode, color: '#34D399', verdict: 'Strong Hire', rating: 93, comment: 'Deep understanding of Java internals, DBMS normalization, and REST API design principles. Live coding showed spontaneous optimization from O(N²) → O(N). Highly recommended for Deloitte Associate Analyst track.' },
  { role: 'HR Panel', icon: FaUsers, color: '#F472B6', verdict: 'Hire', rating: 96, comment: 'Outstanding communication with well-structured STAR responses. Genuine alignment with Deloitte\'s values of innovation and flexibility. Showed strong career intent and adaptability.' },
];
...
const DeloittePrepDetail = () => {
  ...
  const [code, setCode] = useState('// Deloitte Coding Assessment\n// Problem: Longest Palindromic Substring\n\n');
  ...
  const [managerialStep, setManagerialStep] = useState(-1);
  ...
  const startManagerial = () => {
    setManagerialStep(0);
    setMsgs([{ from: 'ai', text: MANAGERIAL_FLOW[0].q, step: MANAGERIAL_FLOW[0].step }]);
  };
    ...
    setTimeout(() => {
      if (activeTab === 'tech') {
        if (techStep < TECH_FLOW.length - 1) {
          ...
          setMsgs(p => [...p, { from: 'ai', text: 'Outstanding performance! That concludes your technical round. Our panel will deliberate shortly. Please proceed to the Managerial Interview.', step: 'Completed' }]);
          setIntDone(p => ({ ...p, tech: true })); setEvalRunning(false); setPipelineStep(4);
        }
      } else if (activeTab === 'managerial') {
        if (managerialStep < MANAGERIAL_FLOW.length - 1) {
        ...
        if (hrStep < HR_FLOW.length - 1) {
          ...
          setMsgs(p => [...p, { from: 'ai', text: "Thank you so much for your time today! It was truly a pleasure speaking with you. The Deloitte Hiring Committee will review your complete profile and you'll receive an update within 72 hours. All the very best!", step: 'Completed' }]);
          setIntDone(p => ({ ...p, hr: true })); setEvalRunning(false); setPipelineStep(6);
        }
      }
    }, 1400);
  };
  ...
  const dlPDF = () => { toast.success('Generating Deloitte Readiness Report PDF…', { icon: '📄' }); setTimeout(() => toast.success('Deloitte_Associate Analyst_Report_2026.pdf downloaded!', { icon: '✅' }), 1800); };
              ...
              <div className="flex flex-col lg:flex-row items-start lg:items-center gap-8 mb-10">
                ...
                <div>
                  <div className="flex items-center gap-3 mb-2 flex-wrap">
                    <h1 className="text-4xl font-black text-white tracking-tight">Deloitte Campus Hiring Simulation</h1>
                  ...
                  <p className="text-sm max-w-2xl leading-relaxed" style={{ color: W.muted }}>
                    End-to-end AI-powered simulation for Technology Consulting · Analyst · Associate Analyst tracks. Proctored assessment, dynamic AI interviews, live scorecard, and personalized analytics — all in one platform.
                  </p>
                </div>
              </div>
          ...
          <div className="max-w-7xl mx-auto px-8 pt-6 space-y-8">
            ...
            {activeTab === 'dashboard' && (
              <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="space-y-8">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <StatCard label="Application Status" value="In Progress" sub="Track: Deloitte Associate Analyst" icon={FaChartLine} color={W.accent} />
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  <div className="rounded-2xl border p-8 space-y-6" style={{ backgroundColor: W.card, borderColor: W.border }}>
                    ...
                    <ProgressBar label="Overall Deloitte Readiness" value={91} color={W.accent} sub="Combined across all rounds" />
                  </div>
                  <div className="rounded-2xl border p-8 space-y-5" style={{ backgroundColor: W.card, borderColor: W.border }}>
                  ...
                  <div className="flex items-start gap-5">
                    ...
                    <div>
                      ...
                      <p className="text-sm leading-relaxed" style={{ color: W.muted }}>
                        You are performing strongly in Assessment and Coding. Deloitte Associate Analyst readiness is at <strong className="text-white">91%</strong>.
            ...
            {activeTab === 'tracks' && (
              <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="space-y-10">
                <div>
                  <h2 className="text-xl font-black text-white mb-6 flex items-center gap-3"><FaTrophy style={{ color: W.accent }} /> Deloitte Hiring Tracks</h2>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    {TRACKS.map((t, i) => (
            ...
            {activeTab === 'pipeline' && (
              <motion.div initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }} className="py-10">
                <h2 className="text-2xl font-black text-white text-center mb-20">Deloitte Recruitment Pipeline</h2>
                        ...
                        <div className="flex flex-col h-full">
                          ...
                          <div className="flex flex-1 overflow-hidden">
                            <div className="w-1/3 p-6 border-r overflow-y-auto space-y-4" style={{ borderColor: W.border }}>
                              <h4 className="font-black text-white text-sm">Longest Palindromic Substring — Deloitte Associate Analyst</h4>
            ...
            {activeTab === 'qbank' && (
              <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="space-y-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                  {(COMPANY_CODING_PYQS['Deloitte'] || COMPANY_CODING_PYQS['Deloitte']).map((q, i) => (
                    <motion.div key={i} whileHover={{ y: -3 }} className="rounded-2xl border p-6 space-y-4 group cursor-pointer relative overflow-hidden" style={{ backgroundColor: W.card, borderColor: W.border }}>
                      ...
                      <div className="relative z-10">
                        <h4 className="font-black text-white group-hover:text-emerald-400 transition-colors">{q.title}</h4>
            ...
            {/* ════════════ TECH / HR INTERVIEW ════════════ */}
            {(activeTab === 'tech' || activeTab === 'hr' || activeTab === 'managerial') && (() => {
              const isTech = activeTab === 'tech';
              const isManagerial = activeTab === 'managerial';
              const isHR = activeTab === 'hr';
              const step = isTech ? techStep : isManagerial ? managerialStep : hrStep;
              const flow = isTech ? TECH_FLOW : isManagerial ? MANAGERIAL_FLOW : HR_FLOW;
              return (
                  ...
                  <div className="flex-1 flex flex-col rounded-2xl border overflow-hidden" style={{ backgroundColor: W.card, borderColor: W.border }}>
                    <div className="p-5 border-b flex items-center justify-between bg-black/30" style={{ borderColor: W.border }}>
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <div className="font-black text-sm text-white">Deloitte AI {isTech ? 'Technical' : isManagerial ? 'Managerial' : 'HR'} Panelist</div>
                    ...
                    <div className="flex-1 overflow-y-auto p-6 space-y-5 custom-scrollbar">
                      {step === -1 ? (
                        <div className="h-full flex flex-col items-center justify-center text-center gap-6">
                          ...
                          <h3 className="text-3xl font-black text-white">{isTech ? 'Technical Panel Round' : isManagerial ? 'Managerial Interview Round' : 'HR Discussion Round'}</h3>
                          <p className="text-sm max-w-md leading-relaxed" style={{ color: W.muted }}>
                            {isTech ? 'A 10-step AI technical interview covering your project, Java, DBMS, OS, CN, live coding, and system concepts — calibrated for Deloitte Associate Analyst.' : isManagerial ? 'A 5-step behavioral assessment analyzing leadership, decision making, conflict resolution, and client management.' : 'A structured 10-topic HR conversation evaluating communication, Deloitte cultural alignment, and career goals using the STAR framework.'}
                          </p>
                          <button onClick={isTech ? startTech : isManagerial ? startManagerial : startHR} className="px-10 py-4 rounded-xl font-black text-white text-sm flex items-center gap-2" style={{ backgroundColor: W.accent, boxShadow: `0 0 25px ${W.accentGlow}` }}>
                    ...
                    <div className="rounded-2xl border p-5 space-y-3" style={{ backgroundColor: W.card, borderColor: W.border }}>
                      ...
                      {flow.slice(0, 6).map((f, i) => (
                        <div key={f.step} className="flex items-center gap-3">
                          ...
                          {i === step && <div className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse ml-auto shrink-0" />}
                        </div>
                      ))}
                    </div>
                    {isTech && (
            ...
            {activeTab === 'panel' && (
              <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="space-y-10 max-w-5xl mx-auto">
                <div className="text-center">
                  ...
                  <p className="text-sm" style={{ color: W.muted }}>Aggregated review across all Deloitte Associate Analyst recruitment rounds.</p>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  {COMMITTEE.map(p => (
                ...
                <div className="rounded-3xl border p-12 relative overflow-hidden" style={{ background: `linear-gradient(135deg, ${W.card} 0%, ${W.accentDark}20 100%)`, borderColor: `${W.accent}50`, boxShadow: `0 20px 60px ${W.accentGlow}` }}>
                  ...
                  <div className="relative z-10 flex flex-col items-center text-center gap-8">
                    ...
                    <div>
                      ...
                      <p className="text-sm" style={{ color: W.muted }}>Unanimous recommendation — Deloitte Associate Analyst Track</p>
                    </div>
                    <div className="w-full rounded-2xl border p-8 grid grid-cols-2 md:grid-cols-4 gap-6 text-left" style={{ backgroundColor: 'rgba(0,0,0,0.3)', borderColor: `${W.accent}20` }}>
                      {[['Company', 'Deloitte Technologies'], ['Role / Track', 'Associate Analyst'], ['Compensation', '9.5 LPA', '#34D399'], ['Joining Batch', 'August 2026']].map(([l, v, col]) => (
                        <div key={l}>
                ...
                {finalResult === 'selected' && (
                  <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="rounded-2xl border p-10 text-center space-y-6" style={{ backgroundColor: '#10B98110', borderColor: '#10B98130' }}>
                    ...
                    <p className="text-sm" style={{ color: W.muted }}>Your Deloitte Associate Analyst offer has been accepted. Joining documentation will arrive within 48 hours.</p>
                ...
                {finalResult === 'not-selected' && (
                  <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="rounded-2xl border p-10 space-y-6" style={{ backgroundColor: '#EF444410', borderColor: '#EF444430' }}>
                    ...
                    <p className="text-sm" style={{ color: W.muted }}>Don't be discouraged — most successful Deloitte Associate Analyst candidates needed 2+ attempts. Here's your plan:</p>
              ...
              <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="space-y-8 max-w-6xl mx-auto">
                <div className="flex justify-between items-end flex-wrap gap-4">
                  <div>
                    ...
                    <p className="text-sm mt-1" style={{ color: W.muted }}>Deloitte Associate Analyst — Simulation Cycle 2026</p>
                  </div>
                  <div className="flex gap-3">
                  ...
                  <div className="space-y-5">
                    <div className="rounded-2xl border p-8 text-center space-y-4 relative overflow-hidden" style={{ backgroundColor: W.card, borderColor: `${W.accent}40`, boxShadow: `0 0 30px ${W.accentGlow}` }}>
                      <div className="relative z-10">
                        ...
                        <div className="text-xs mt-1" style={{ color: W.muted }}>Deloitte Associate Analyst Track</div>
                      </div>
                    </div>
                    <div className="rounded-2xl border p-6 space-y-4" style={{ backgroundColor: W.card, borderColor: W.border }}>
                      ...
                      <tbody>
                        {HISTORY.map((r, i) => (
                          <tr key={i} className="border-b hover:bg-white/5 transition-colors" style={{ borderColor: W.border }}>
                            ...
                            <td className="px-5 py-4 font-bold text-green-400">{r.oa}</td>
...
export default DeloittePrepDetail;
